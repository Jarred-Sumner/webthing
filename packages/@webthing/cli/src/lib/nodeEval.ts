 ...
 * @returns {*}
 */
export const nodeEval = (content, filename, context, paths: Array<string>) => {
  ...
  if (/\b(exports|module)\b/.test(content)) {
    sandbox = _commonjsEval(content, filename, context, paths);
  }
...
function _commonjsEval(content, filename, context, paths: Array<string>) {
  ...
  if (filename) {
    ...
    sandbox.require.resolve = req => {
      try {
        return Module._resolveFilename(req, sandbox.module);
      } catch (exception) {
        return __non_webpack_require__.resolve(req, {
          paths
        });
      }
    };
  ...
  context &&
    (contextKeys = Object.keys(context).map(key => {
...
function wrap(body, extKeys) {
  ...
  const wrapper = [`(function (${keys}`, ") { ", "\n});"];

  return wrapper[0] + wrapper[1] + body + wrapper[2];
}
...
function parseStackLine(line) {
  ...
  const filename =
    ["eval", "<anonymous>"].indexOf(parts[1]) > -1 ? undefined : parts[1];
